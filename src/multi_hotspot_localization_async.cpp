...
private:
    ...
    sensor_msgs::PointCloud2ConstPtr latest_pointcloud_msg;
...
public:
    RealSenseFireLocalization() : tfListener(tf_buffer)
    {
        ...
        if (!nh.getParam("/fire/localization/hotspot/threshold", hotspot_threshold)) {
            ROS_WARN("[FIRE LOCALIZATION] Parameter /fire/localization/hotspot/threshold not set, using default value of 24500");
            nh.setParam("/fire/localization/hotspot/threshold", 24500);
            hotspot_threshold = 24500;
        }

        if (!nh.getParam("/fire/localization/cluster/min", cluster_size_min)) {
            ROS_WARN("[FIRE LOCALIZATION] Parameter /fire/localization/cluster/min not set, using default value of 100");
            nh.setParam("/fire/localization/cluster/min", 100);
            cluster_size_min = 100;
        }

        if (!nh.getParam("/fire/localization/hotspot/points_min", hotspot_points_min)) {
            ROS_WARN("[FIRE LOCALIZATION] Parameter /fire/localization/hotspot/points_min not set, using default value of 10");
            nh.setParam("/fire/localization/hotspot/points_min", 10);
            hotspot_points_min = 10;
        }

        // viz pub for mask clusters
        mask_clusters_pub = nh.advertise<sensor_msgs::Image>("/rsun/hotspot/masks", 1);
        fire_pointcloud_pub = nh.advertise<sensor_msgs::PointCloud2> ("/rsun/hotspot/points", 1);

        // Timer for periodic processing
        processing_timer = nh.createTimer(ros::Duration(0.033*3), &RealSenseFireLocalization::timerCallback, this);
    }
    ...
    void thermalImageCallback(const sensor_msgs::ImageConstPtr& msg) {
        try {
            ...
            nh.getParam("/fire/localization/hotspot/threshold", hotspot_threshold);
            ...
            // ROS_INFO("[FIRE LOCALIZATION] clusters visible: %zu", mask_clusters.size());
            ...
            ROS_ERROR("[FIRE LOCALIZATION] cv_bridge exception: %s", e.what());
        }
    }
    ...
    void findClusters(const cv::Mat& binary_image, std::vector<cv::Mat>& clusters) {
        clusters.clear();
        nh.getParam("/fire/localization/cluster/min", cluster_size_min);
    ...
    void timerCallback(const ros::TimerEvent&) {
        ...
        try {
            thermal_realsense_transform = tf_buffer.lookupTransform("flir_boson_optical_frame", latest_pointcloud_msg->header.frame_id, ros::Time(0));
        } catch (tf2::TransformException &ex) {
            ROS_WARN("[FIRE LOCALIZATION] %s", ex.what());
            return;
        }

        // Transform point cloud
        pcl_ros::transformPointCloud("flir_boson_optical_frame", thermal_realsense_transform.transform, *latest_pointcloud_msg, transformed_points);
        ...
        nh.getParam("/fire/localization/hotspot/points_min", hotspot_points_min);
        if (!filtered_points.empty()) {
            ...
            for (size_t i = 0; i < filtered_points.size(); ++i) {
                // reject clusters with less than hotspot_points_min points
                if (filtered_points[i]->points.size() < hotspot_points_min) {
                    continue;
                }
                ...
                if (!filtered_points[i]->points.empty()) {
                    ...
                    // print the number of points inside the cluster
                    ROS_INFO("[FIRE LOCALIZATION] points inside cluster %zu: %zu", i, filtered_points[i]->points.size());
                }
            }

            // Publish the PoseArray message
            hotspots_pose_array_pub.publish(pose_array_msg);
            ROS_INFO("[FIRE LOCALIZATION] hotspots found and pose published: %zu", pose_array_msg.poses.size());
        }
        else {
            ROS_INFO("[FIRE LOCALIZATION] no hotspots found in thermal image");
        }
    }
};
...
{
    ros::init(argc, argv, "multi_hotspot_localization_async");
