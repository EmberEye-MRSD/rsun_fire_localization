...
public:
    ...
    // void findClusters(const cv::Mat& binary_image, std::vector<std::vector<cv::Point2d>>& clusters) {
    //     // Create a copy of the binary image to avoid modifying the original
    //     cv::Mat labeled_image;
    //     binary_image.copyTo(labeled_image);
    ...
    //         std::vector<cv::Point2d> cluster_points;
    //         for (int row = 0; row < labels.rows; ++row) {
    //             for (int col = 0; col < labels.cols; ++col) {
    //                 if (labels.at<int>(row, col) == label) {
    //                     cluster_points.emplace_back(cv::Point2d(col, row));
    //                 }
    //             }
    //         }
    ...
    //     clusters.erase(std::remove_if(clusters.begin(), clusters.end(), [](const std::vector<cv::Point2d>& cluster) {
...
// int main() {
//     // Example usage
//     cv::Mat binary_image = cv::imread("/home/gangadhar-nageswar/Downloads/multi.jpg", cv::IMREAD_GRAYSCALE);

//     // Invert the image to ensure black circles on white background become white clusters on black background
//     cv::bitwise_not(binary_image, binary_image);

//     // Threshold the image to ensure it's binary
//     cv::threshold(binary_image, binary_image, 127, 255, cv::THRESH_BINARY);

//     std::vector<std::vector<cv::Point2d>> clusters;
//     findClusters(binary_image, clusters);
